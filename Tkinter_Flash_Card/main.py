...
def flip_card():
    ...
    canvas.itemconfig(word_text, text=card_manager.current_word["english"], fill="white")

def next_language():
    global flip_timer
    window.after_cancel(flip_timer)
    card_manager.pickOne()
    canvas.itemconfig(canvas_image, image=card_front)
    canvas.itemconfig(title_text, text="Japanese", fill="black")
    canvas.itemconfig(word_text, text=card_manager.current_word["trans"], fill="black")
...
right_button = tk.Button(image=right_img, highlightthickness=0, border=0, command=guessed_correct)
...
canvas.itemconfig(word_text, text=card_manager.current_word["trans"])
